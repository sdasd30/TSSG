...
public class ImpressionModifier 
{
        ...
        int m_maxStack = 1, float stack_buffer_time = 1.0f, int starting_stack  = 1, bool m_isRefreshable = true)
    {
        ...
        if (duration < 0)
            m_isTimed = false;
        m_expirationDate = ScaledTime.UITimeElapsed + duration;
        this.m_isRefreshable = m_isRefreshable;
        m_lastApplyTime = ScaledTime.UITimeElapsed;
    ...
    public bool is_expired()
    {
        ...
        return !m_isTimed || ScaledTime.UITimeElapsed > m_expirationDate;
    }
    ...
    public bool attempt_stack(int stackVal = 1)
    {
        if (!m_isRefreshable)
            return false;
        if (ScaledTime.UITimeElapsed < m_lastApplyTime + m_stackBufferTime)
            return false;
        m_lastApplyTime = ScaledTime.UITimeElapsed;
        m_expirationDate = ScaledTime.UITimeElapsed + duration;
