...
[System.Serializable]
public class ActionMovementInfo
{
...
public class HitboxInfo {
    ...
    {
        HitboxScale = new Vector3(1.0f, 1.0f, 1.0f);
        HitboxOffset = new Vector3(0f, 0f);
        Damage = 10.0f;
        FocusDamage = -1f;
        Penetration = 0.0f;
        Stun = 0.3f;
        HitboxDuration = 0.5f;
        Knockback = new Vector3(10.0f, 10.0f, 0f);
        FixedKnockback = true;
        ResetKnockback = true;
...
public class ActionInfo : MonoBehaviour
{
    ...
    public ActionMovementInfo m_MovementInfo;
	...
	protected void createHitboxes()
	{
		foreach (HitboxInfo hi in m_HitboxInfo) {
			...
			if (hi.Delay <= 0f)
                ...
                m_hitboxMaker.QueueHitbox (hi, hi.Delay);
		}
	}
    ...
    public void AddCallBack(OnRegisterHit registerFunction)
    {
        foreach(HitboxInfo hitboxInfo in m_HitboxInfo)
        {
            hitboxInfo.hitCallback.Add(registerFunction);
        }
    }
    ...
    protected InputPacket getLastInputPacket()
    {
